...
const configuration = {
	...
	callback = function (error, response) {
		...
		response.images.forEach(({name, contents}) => {
			fs.writeFile('./public/' + name, contents, (err) => {if (err) console.error(err)})
		})
		...
		fs.readFile('./template.html', 'utf8', (err, data) => {
			...
			const result = data.replaceAll('%faviconsInfo%', response.html.map(v => '\t' + v).join('\n'));

			fs.writeFile('./index.html', result, 'utf8', (err) => {if (err) console.error(err)});
		});
	};
