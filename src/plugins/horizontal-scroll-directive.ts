...
export const horizontalScrollDirective = {
	mounted: (el, binding, vnode, oldVnode) => {
		const wheel = (e) => {
			...
			if (el.horizontalScrollDirective.sumDelta === 0) el.horizontalScrollDirective.startLeft = el.scrollLeft
			el.horizontalScrollDirective.sumDelta -= e.deltaY
			const maxScrollLeft = el.scrollWidth - el.clientWidth;
			const left = Math.min(maxScrollLeft, Math.max(0, el.horizontalScrollDirective.startLeft - el.horizontalScrollDirective.sumDelta));
			// targetLeft = left
