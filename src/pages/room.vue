		...
		<div :class="[style.slotsGroup]">
			...
			<div v-for="(p, i) in players" :class="[style.slotCtn]">
				<button
					 ...
					 :style="`background: ${['#F7AEF8', '#B388EB', '#8093F1', '#72DDF7'][i % 4]}`"
				...
				<div :class="[style.slotName, {[style.slotNameMe]: p.selfId === player.selfId}]">
					{{ p.data.name }}
				</div>
				<button
					 type=button
...
<script lang=ts>
import {computed, defineComponent, inject, onBeforeMount, onBeforeUnmount, reactive, ref, unref} from "vue";
