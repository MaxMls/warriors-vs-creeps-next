<template>
  <div :class="[cs.ctn, style.ctn]" ref="ctn">
    <div :class="style.header">
      <div :class="style.headerLinks"></div>
      <div :class="style.headerMessage">
        <div :class="style.headerMessageText">
          <div :class="style.headerMessageTextCtn">
            ...
            <span
              v-typer="$t(game.message)"
              :class="style.headerMessageTextRepeat"
            />
          </div>
        </div>
      </div>
    ...
    <div :class="style.statsPanelCtn">
      <div :class="style.statsPanel">
        <div
          :class="[style.statCtn, style.statCtnCounter]"
          v-for="i in ['creep', 'heart', 'time']"
        >
        ...
        >
          <SvgIcon
            v-if="game.scrapType === 'fix'"
            :class="style.statIcon"
            name="fix"
          />
          <SvgIcon
            v-else-if="game.scrapType === 'move'"
            :class="style.statIcon"
            name="cranMove"
          />
    ...
    <div :class="style.handPanelCtn">
      <div :class="style.handPanel">
        ...
        >
          <CardComponent
            :idx="idx"
            :select="!!game.onHandClick"
            :active="game.handActiveCardInd === i"
          />
        </button>
      </div>
    </div>
    <div :class="style.termPanel" :ref="visual.setItemTermPanelRef">
      <div
        v-for="(stack, i) in game.stacks"
        :class="style.termSlot"
        :ref="visual.setItemTermSlotRef"
      >
        <button
          type="button"
    ...
    <div
      v-click-outside
      v-opacity-delay
      v-if="game.rotationsSelect.length"
      :class="style.directionPopupCtn"
    >
      <div :class="style.directionPopup">
        ...
        <div :class="style.directionPopupChoose">
          ...
          >
            <SvgIcon
              :style="
                visual.rotateDirectionStyle(
                  game.rotationsSelectCurrentDirection,
                  rot
                )
              "
              name="direction"
...
const useVisualGame = (game) => {
  ...
  const recalculateTermCardsBottomOffset = () => {
    ...
    for (const slot of termSlotRefs) {
      ...
      const v =
        Math.min(0, ctnHeight - card.getBoundingClientRect().height) + "px";
  ...
  return {
    ...
    rotateDirectionStyle(dir, rot) {
      return `transform: rotate(${directionToDeg(
        rotateDirection(dir, rot)
      )}deg)`;
    },
...
const useRenderGameMap = () => {
  ...
  const updateHeroDirection: AbstractRender["updateHeroDirection"] = (
    cell: Cell,
...
const useRenderGame = () => {
  ...
  const chooseRotate: AbstractRender["chooseRotate"] = (
  ...
  const selectStacks: AbstractAgent["selectStacks"] = (
    stacks: TStackId[],
    count: number
  ) =>
    new Promise<number[]>((resolve, reject) => {
  ...
  const startTimer = (secCount: number): void => {
    ...
    const updateTimer = () => {
      ...
      time.value = date.toLocaleTimeString(undefined, {
        minute: "numeric",
        second: "2-digit"
      });
    };
  ...
  return {
    ...
    onRotationClick,
    rotationsSelect,
    rotationsSelectCurrentDirection,
    message,
    selectsCards,
    onCardClick,
    handCards,
...
const useStartGame = (render: AbstractRender, skins) => {
  ...
  onMounted(async () => {
    ...
    for (const player of players) {
      if (
        player.selfId !== room.player.selfId &&
        player.selfId === player.ownerId
      ) {
        if (!network) {
    ...
    const users = players.map((player) => {
      const agent =
        player.selfId === room.player.selfId
          ? new LocalAgent(network, room.player.selfId, unref(render))
          : player.selfId === player.ownerId
          ? new NetworkAgent(network!, player.selfId, seed)
          : new BotAgent(seed);
