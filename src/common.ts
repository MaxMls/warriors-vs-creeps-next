...
export const dynamicSort = (property) => (a, b) =>
  a[property] < b[property] ? -1 : a[property] > b[property] ? 1 : 0;

export class GlobalEventEmitter {
  constructor(private readonly url, private readonly id = genId()) {}
...
export type TAnswerCallback = (
  data: IAnswerData,
  sendAnswer: (answer: any, name: string) => void
) => void;
...
export class HostRoom {
  ...
  constructor(
    ...
    this.eventRequestFromInit = genEvent(
      "room",
      this.roomId,
      "peer",
      this.peerName,
  ...
  onConnection: ((name: string, connection: RTCPeerConnection) => void) | null =
    null;
  ...
  private async onInitiatorRequest({ initName }) {
    ...
    const eventSignalingFromInit = genEvent(
      "room",
      roomId,
      "peer",
      peerName,
    ...
    const eventSignalingFromPeer = genEvent(
      "room",
      roomId,
      "init",
      initName,
