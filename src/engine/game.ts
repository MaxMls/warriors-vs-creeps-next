...
export class Game {
  ...
  public start() {
    ...
    // Спаун героев
    const baseFree = this.gameMap
      .getAllCellsByType(ETileType.Base)
      .filter((cell) => !cell.hasUnit());
    shakeArray(baseFree, this.random);

    for (const user of this.users) {
      const hero = new Unit(EUnitType.Hero);
      const heroCell = baseFree.pop();
  ...
  private async chooseCards() {
    ...
    const countCards = this.users.length * (this.roundCounter === 0 ? 2 : 1);
      ...
      for (let i = 0; i < countCards; i++) {
        if (selectionCards.length === 0) {
          ...
          this.render.showMessage(
            this.users[userId].current ? "message.0" : "message.1"
          );

          const selectCardInd = await this.users[userId].selectCard(
            selectionCards
          );
    ...
    await Promise.all(
      this.users.map((user) =>
        user.programming().then(() => {
          if (user.current) this.render.showMessage("message.3");
        })
      )
    );
  ...
  private async warriorsAct() {
    for (const user of this.users) {
      this.render.showMessage(user.current ? "message.4" : "message.5");
  ...
  private async runStack(user: User, stack: TCardId[]) {
    ...
    const cardId = stack[level - 1];
    const card = cardsJSON[cardId].levels[level - 1];
    let heroCell = this.gameMap
      .getAllCellHasUnits(EUnitType.Hero)
      .filter((cell) => cell.unit === user.myHero)[0];

    if (card?.rotate?.length) {
      this.render.showMessage(user.current ? "message.6" : "message.7");
      const rotateAngleId =
        card.rotate[
          card.rotate.length > 1
            ? await user.chooseRotate(card.rotate, user.myHero.direction)
            : 0
        ];
    ...
    if (card.move.length !== 0) {
      ...
      if (card.move.length === 1) {
        // card.move[i] - вектор в конец которого нужно дойти
        selVect = card.move[0];
      } else {
        // Выбор или запрос ячейки для передвижения
        const sellArray: Cell[] = [];
        const sellVec: IVector[] = [];
        for (const sel of card.move) {
          const v = vectorRotate(sel, user.myHero.direction);
          const temp = Math.max(Math.abs(v.x), Math.abs(v.y));
          let next = { x: heroCell.x + v.x, y: heroCell.y - v.y };

          while (true) {
            const nextCell = this.gameMap.getCell(next.x, next.y);
        ...
        if (sellVec.length === 0) {
          ...
          const moveCellIds = await user.selectCells(
            sellArray,
            EHighlight.Move,
            1
          );
      ...
      if (selVect !== null) {
        const v = vectorRotate(selVect, user.myHero.direction);
        await this.goRamming(user, heroCell, v.x, -v.y);
      }
    }

    heroCell = this.gameMap
      .getAllCellHasUnits(EUnitType.Hero)
      .filter((cell) => cell.unit === user.myHero)[0];
  ...
  private async goRamming(user: User, thisCell: Cell, vecX, vecY) {
    ...
    for (const hook of hookVectors) {
      const hookTemp = this.gameMap.getCell(
        thisCell.x + hook.x,
        thisCell.y - hook.y
      );
      if (
        hookTemp !== null &&
        hookTemp.hasUnit() &&
        (hookTemp.unit.type === EUnitType.Hero ||
          hookTemp.unit.type === EUnitType.Bomb)
      ) {
        hookArray.push(hookTemp);
      }
    }
    hookArray.push(thisCell);
    const unit = thisCell.unit;

    if (hookArray.length > 1) {
      this.render.showMessage(user.current ? "message.10" : "message.11");
      hookSelect =
        hookArray[(await user.selectCells(hookArray, EHighlight.Hook, 1))[0]];
      if (hookSelect !== thisCell) {
        thisCell.unit.attachedCell = hookSelect;
      }
    }

    const toX = thisCell.x + vecX;
    const toY = thisCell.y + vecY;
    const temp = Math.max(Math.abs(vecX), Math.abs(vecY));

    // Развернутая рекурсия
    const recStack: Cell[] = [];
    recStack.push(thisCell); // клетка которую двигаем
    const stopMatrix = createArray(this.gameMap.size.x, this.gameMap.size.y);
    ...
    while (recStack.length) {
      ...
      const next = this.gameMap.getCell(
        (curCell.x + vecX / temp) | 0,
        (curCell.y + vecY / temp) | 0
      );

      // Можно ли двигать дальше
      if (
        next === null ||
        (curCell.x === toX && curCell.y === toY) ||
        stopMatrix[next.x][next.y] == true
      ) {
      ...
      if (
        next.unit !== null &&
        (next.unit.type === EUnitType.Hero || next.unit.type === EUnitType.Bomb)
      ) {
      ...
      if (next.unit !== null && next.unit.type === EUnitType.Creep)
        this.creepKill(next);
      ...
      if (next.unit.attachedCell !== null) {
        const atCell = next.unit.attachedCell;
        if (next.unit.attachedCell !== next && atCell.unit) {
          // todo bug: it should be impossible, but not
  ...
  private async goAttack(user, thisCell, attackVecs, count) {
    ...
    for (const vec of attackVecs) {
      const dirVec = vectorRotate(vec, thisCell.unit.rotation);
      const tempCell = this.gameMap.getCell(
        dirVec.x + thisCell.x,
        thisCell.y - dirVec.y
      );
      if (
        tempCell !== null &&
        tempCell.unit !== null &&
        tempCell.unit.type === EUnitType.Creep
      ) {
    ...
    if (attArray.length > count) {
      ...
      this.render.showMessage(user.current ? "message.13" : "message.14");
      const atIds = await user.selectCells(attArray, EHighlight.Attack, count);
  ...
  private async creepsMoveAct() {
    // TODO: damage unit hero if on path
    this.render.showMessage("message.15");

    const creepsCells = this.gameMap.getAllCellHasUnits(EUnitType.Creep);
    const bombCells = this.gameMap.getAllCellHasUnits(EUnitType.Bomb);

    const movements: Promise<void>[] = [];
    for (const cellFrom of creepsCells) {
      const next = getNextCellFromAToB(cellFrom, bombCells[0]);

      const to = this.gameMap.moveUnitFromCellToCoords(
        cellFrom,
        next.x,
        next.y
      );
  ...
  private creepsSpawn() {
    const runesFree = this.gameMap
      .getAllCellsByType(ETileType.Runes)
      .filter((cell) => !cell.hasUnit());
    for (const spawnCell of runesFree) {
  ...
  private creepsAttackAct() {
    const bombs = this.gameMap.getAllCellHasUnits(EUnitType.Bomb);
    const heroCells = this.gameMap.getAllCellHasUnits(EUnitType.Hero);

    const sortHeroCells: Cell[] = [];
    for (const user of this.users) {
    ...
    const attackedCells = bombs.concat(sortHeroCells);

    const attackEvent: { attacking: Cell; attacked: Cell }[] = [];
    const crossVectorsZone = [
      [-1, 0],
      [1, 0],
      [0, -1],
      [0, 1],
    ...
    for (const cell of attackedCells) {
      for (const vec of crossVectorsZone) {
        const creepCell = this.gameMap.getCell(
          cell.x + vec[0],
          cell.y + vec[1]
        );
        if (
          creepCell !== null &&
          creepCell.unit !== null &&
          creepCell.unit.type === EUnitType.Creep
        ) {
    ...
    const attack = (eventId = 0) => {
      const atEv = attackEvent[eventId];
  ...
  private getDisable(user: User) {
    if (this.damageCardsDeck.length !== 0) {
      const ranId = getRandomInt(this.random, 0, 6);
      const damageCard = this.damageCardsDeck.pop() as TCardId; // because check length on top
      if (
        user.stacks[ranId].length > 0 &&
        cardsJSON[user.stacks[ranId][user.stacks[ranId].length - 1]].type ===
          ECardType.Defect
      ) {
  ...
  private finalAct() {
    ...
    if (
      !this.gameMap
        .getAllCellHasUnits(EUnitType.Bomb)
        .find((cell) => cell.type !== ETileType.Target)
    ) {
      this.win();
    } else {
