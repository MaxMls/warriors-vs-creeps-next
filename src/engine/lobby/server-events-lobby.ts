...
export class Room {
	...
	public async openRoom(): Promise<string> {
		if (!this.openRoomId) {
			...
			const openUserId = await this.networkEvents.on(['room', this.openRoomId], this.roomListenerThis)
	...
	private updatePlayerDataLocal(id: IRoomPlayer['selfId'], value: IRoomPlayer['data']) {
	...
	private removePlayerLocal(id: IRoomPlayer['selfId']) {
		this.players = this.players.filter(v => v.selfId !== id && v.ownerId !== id)
		this.localEvents.emit('players')
	}
	...
	private roomListener({type, id, data}) {
		if (id === this.player.selfId) {
			...
			this.roomEmitter('addPlayer_NE', this.player)
			this.players.filter(p => p.ownerId === this.player.selfId).forEach((p) => {
