...
export class User {
  ...
  async selectCard(cards: TCardId[]) {
    // Запрос пользователю выбрать x карт
  ...
  private get stacksNoDefects() {
    const notDisabledStacks: TStackId[] = [];

    for (let i = 0; i < 6; i++) {
      if (
        this.stacks[i].length === 0 ||
        cardsJSON[this.stacks[i][this.stacks[i].length - 1]].type !==
          ECardType.Defect
      ) {
  ...
  private get stacksNoDefectsNoEmpty() {
    const notDisabledNoEmpty: TStackId[] = [];
  ...
  async scrapRequest(type) {
    ...
    if (type === ECardType.Fire || type === ECardType.Metal) {
      const disabledStacks = this.stacksDefects;

      if (disabledStacks.length > 0) {
        const defectId =
          disabledStacks[(await this.agent.selectStacks(disabledStacks, 1))[0]];
    ...
    // Утилизация электрических или компьютерных карт для перепрограммирования - свап 2-х активных стеков на выбор
      ...
      if (allowedStacksFrom.length >= 2) {
        ...
        const fromId =
          allowedStacksFrom[
            (await this.agent.selectStacks(allowedStacksFrom, 1))[0]
          ];
        const toId =
          allowedStacksTo[
            (await this.agent.selectStacks(allowedStacksTo, 1))[0]
          ];

        const tmpStack = this.stacks[fromId];
  ...
  async programming() {
    ...
    while (this.hand.length > 0) {
      ...
      if (stackId === -2) {
        // КАРТЫ если она УТИЛИЗИРУЕТСЯ БЕЗ ЭФФЕКТА
        this.hand.splice(cardPosInHand, 1);
      } else if (stackId === -1) {
        // Карты если она УТИЛИЗИРУЕТСЯ С ЭФФEКТОМ
      ...
      // усилить стек картой того же типа
        ...
        this.stacks[stackId].length === 0 ||
        cardsJSON[this.stacks[stackId][0]].type ===
          cardsJSON[this.hand[cardPosInHand]].type
      ) {
